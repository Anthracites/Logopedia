...
using System.IO;
using System.Linq;
using Assets.SimpleLocalization.Editor;
...
namespace Assets.SimpleLocalization.Scripts.Editor
{
    public class LocalizationEditorWindow : EditorWindow
    {
        ...
        private static void MakeLanguagesRow()
        {
            ...
            GUILayout.Label("Key" + (Editor.CurrentKey == "" ? "" : Editor.Keys[Editor.CurrentKey] != Editor.CurrentKey ? " *" : ""), style, GUILayout.MinWidth(200), GUILayout.MinHeight(20));
            GUILayout.EndHorizontal();

            foreach (var language in Editor.SheetDictionary.Keys)
            {
                GUILayout.BeginHorizontal("box", GUILayout.MinWidth(_columnWidth - 30));
                GUILayout.Label(language + (language == LocalizedTranslate.SourceLanguage ? " [Primary]" : ""), style, GUILayout.MinWidth(_columnWidth - 28), GUILayout.MinHeight(20));
        ...
        private static void MakeSheetRow(string key)
        {
            ...
            var newValue = EditorGUILayout.TextField(Editor.Keys[key], GUILayout.MinHeight(50), GUILayout.MaxWidth(200), GUILayout.MinWidth(MinColumnWidth + 7)); 
        ...
        private void MakeKeysDropdown()
        {
            ...
            {
                EditorCoroutineUtility.StartCoroutine(LocalizedTranslate.TranslateAuto(), this);
            }
        ...
        public void MakeBottomMenu()
        {
            ...
            if (GUILayout.Button("Revert") && EditorUtility.DisplayDialog("Simple Localization", "Do you want to revert changes?", "Yes", "No"))
            {
                Editor.ResetSheet();
            }
